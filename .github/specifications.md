# Machine Learning Application to Real-World Data

**Major Course Output**

- Release Date: January 12, 2026
- Due Date: March 21, 2026 (Saturday), 8:00 AM
...
- **LO1.** Define and differentiate the various machine learning techniques and approaches.
- **LO2.** Assess the impact of the design and parameters of various machine learning models in the context of a machine learning task.
- **LO3.** Apply machine learning approaches on various tasks on real-world datasets, and effectively communicating findings and results.

---

## 2 Groupings

The project must be accomplished in groups of 4. If the class size is not divisible by 4, the instructor is responsible for balancing the number of students in each group, but no group can have more than 4 members. Students may pick their own group members.
...
1. Although the task is not required to be socially impactful, the students must be able to justify why this task is interesting to explore on a personal level, i.e., the task must be something that the students care about, not just selected out of convenience or compliance.
2. The task must be relatively unexplored. Avoid selecting tasks that have already been extensively studied, either in academic works or in blogs, tutorials and other online resources. While it is not required for the task to be completely novel, students must avoid simply replicating existing works. Toy datasets are not allowed.
...
We suggest looking for datasets in dataset-focused academic journals such as _Data in Brief_ and _Scientific Data_. You are not required to use datasets from these journals, however. Online data repositories such as Kaggle and UCI Machine Learning Repository can serve as alternative sources, but please use caution since many of the datasets there have already been extensively studied.
...
For this project, you are required to implement **at least three supervised machine learning models: two classical machine learning models and one neural network model**. The choice for the type of classical models and neural network model must be justified.
...
A poster must be prepared summarizing the methodology and results of the project. The poster must concisely include the following information:

1. The goal of the project
2. Description of the dataset used
3. The best machine learning model implemented and architecture details
4. Key results and findings
5. Any other strong or unique aspects of the project

You are encouraged to use visuals and figures to effectively communicate your results. You should only put your best model on the poster. If there are multiple models that performed well and there is no clear best model, you may include a comparison table of the top models.

The poster must be designed to fit on one whole illustration board (30 inches × 40 inches). You may use any software or tools to create the poster. Ensure that when printed, the poster is readable, even though you are only required to submit a digital copy.

---

## 6 Submission

All deliverables must be submitted via AnimoSpace before **March 21, 2026 (Saturday), 8:00 AM**. The submission must include two files: a `.zip` file containing the Jupyter Notebook and all supporting files, and a `.pdf` file of the poster. If the dataset files are small enough (less than 100 MB), you may include them in the `.zip` file. Otherwise, please provide a link to a public repository where the dataset can be downloaded.
